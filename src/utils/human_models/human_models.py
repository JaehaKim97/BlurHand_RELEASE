...
from utils.human_models.smplx import smplx

HUMAN_MODEL_PATH = osp.join(osp.dirname(os.path.abspath(__file__)), 'human_model_files')
...
class MANO(object):
    def __init__(self, flat_hand_mean=False):
        ...
        self.layer = {'right': smplx.create(HUMAN_MODEL_PATH, 'mano', is_rhand=True, use_pca=False, flat_hand_mean=flat_hand_mean, **self.layer_arg), 'left': smplx.create(HUMAN_MODEL_PATH, 'mano', is_rhand=False, use_pca=False, flat_hand_mean=flat_hand_mean, **self.layer_arg)}
        self.vertex_num = 778
